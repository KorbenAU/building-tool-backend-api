...
namespace Microservice.Business.Business.Concrete
{
    public class Registration : IRegistration
    {
         ...
         public int? Register(RegisterModel model, string ipAddress, out string errorMessage)
         {
             ...
             {
                 Log.Information("Registration attempted email address or username that is already in use from IP {0}", ipAddress);
             ...
             {
                 Log.Information("Registration attempted with password that does not meet the passwod policy from IP {0}", ipAddress);
